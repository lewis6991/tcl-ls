...
class MissingOptionValueChecker(DiagnosticChecker):
    CODE: ClassVar[str] = 'missing-option-value'
    ...
    def check(self, context: DiagnosticContext) -> Iterable[Diagnostic]:
        for command_call, command_target in resolved_command_calls(context):
            ...
            yield self.emit(
                span=span,
                message=f'Option `{option_name}` for command `{command_name}` requires a value.',
            )
