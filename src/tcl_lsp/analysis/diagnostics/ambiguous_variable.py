...
class AmbiguousVariableChecker(DiagnosticChecker):
    CODE: ClassVar[str] = 'ambiguous-variable'
    ...
    def check(self, context: DiagnosticContext) -> Iterable[Diagnostic]:
        for resolved_variable in context.variable_resolutions:
            ...
            if (
                resolved_variable.resolution.uncertainty.state != 'ambiguous'
                or variable_reference.procedure_symbol_id is None
            ):
                continue
            yield self.emit(
                span=variable_reference.span,
                message=f'Variable `{variable_reference.name}` resolves to multiple bindings.',
            )
